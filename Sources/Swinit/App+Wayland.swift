...
extension App {
    ...
    func platformWindowRemoved(id: WindowId) {
        ...
        if pointerWindow?.id  == id { pointerWindow  = nil; csdRouter.reset() }
        ...
        pointer?.onEvent = { [weak self] event in
            ...
            case .enter(_, let surface, let x, let y):
                ...
                if let win = findWindow(bySurfaceId: surface.id) {
                    ...
                    win.dispatch(.cursorMoved(deviceId: .placeholder, position: PhysicalPosition(x, y)))
        ...
        keyboard?.onEvent = { [weak self] event in
            ...
            case .key(_, _, let key, let state):
                if let win = keyboardWindow {
                    let ev = KeyEvent(physicalKey: key, logicalKey: key,
                                      state: state == .pressed ? .pressed : .released,
                                      isRepeat: false)
...
struct CSDInputRouter: ~Copyable {
    ...
    mutating func move(x: Double, y: Double) { self.x = x; self.y = y }
    mutating func reset() { activeWindow = nil; activeArea = nil }
}
