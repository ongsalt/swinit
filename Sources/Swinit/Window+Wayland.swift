...
extension Window {
    ...
    func platformSetTitle(_ title: String) {
        ...
        if let csd, let shm = app?.waylandShm {
            try? csd.update(shm: shm, contentSize: _size, title: title,
    ...
    func setActivated(_ active: Bool) {
        ...
        isActivated = active
        try? csd.update(shm: shm, contentSize: _size, title: _title,
    ...
    func handleCSDPress(area: CSDArea, x: Double, y: Double, seat: WlSeat, serial: UInt32) {
        ...
        case .titleBar:
            ...
            func hit(_ c: SIMD2<Double>) -> Bool {
                let dx = x - c.x; let dy = y - c.y; return dx * dx + dy * dy <= r * r
            }
            if      hit(csd.closeCenter)    { dispatch(.closeRequested) }
    ...
    func setupDecorations() {
        ...
        case .auto:
            if let decoManager = app?.waylandDecoManager {
                guard let deco = try? decoManager.getToplevelDecoration(toplevel: toplevel) else {
                    ensureCSD()
                    return
                }
                toplevelDeco = deco
