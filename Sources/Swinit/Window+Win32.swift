...
extension Window {
    ...
    public var titleBarAppearance: TitleBarAppearance {
        ...
        set { _titleBarAppearance = newValue; applyTitleBarAppearance() }
    }
    ...
    func setupWin32Platform(attributes: WindowAttributes) {
        ...
        SetWindowPos(handle, nil, 0, 0, 0, 0,
                     UInt32(SWP_NOSIZE) | UInt32(SWP_NOMOVE) |
                     UInt32(SWP_DRAWFRAME) | UInt32(SWP_SHOWWINDOW))
    }
    ...
    func platformRequestResize(to size: Size) {
        SetWindowPos(handle, nil, 0, 0, Int32(size.width), Int32(size.height),
                     UINT(SWP_NOMOVE | SWP_NOZORDER))
    }

    func platformFocus() { SetForegroundWindow(handle); SetFocus(handle) }
    ...
    func wndProc(_ message: UINT, _ wParam: WPARAM, _ lParam: LPARAM) -> LRESULT {
        ...
        case UINT(WM_ENTERSIZEMOVE):
            isResizing = true
            currentHeartbeatTimer = SetTimer(handle, UInt64.random(in: .min ... .max),
        ...
        case UINT(WM_EXITSIZEMOVE):
            ...
            let sz = Size(width: UInt32(max(0, rect.right - rect.left)),
        ...
        case UINT(WM_SETFOCUS):  dispatch(.focused(true));  return 0
        case UINT(WM_KILLFOCUS): dispatch(.focused(false)); return 0
        ...
        case UINT(WM_LBUTTONDOWN), UINT(WM_RBUTTONDOWN),
            ...
            dispatch(.mouseInput(deviceId: .placeholder, state: .pressed,
                                 button: mouseButton(from: message, wParam: wParam)))
            return 0

        case UINT(WM_LBUTTONUP), UINT(WM_RBUTTONUP),
             UINT(WM_MBUTTONUP), UINT(WM_XBUTTONUP):
            dispatch(.mouseInput(deviceId: .placeholder, state: .released,
                                 button: mouseButton(from: message, wParam: wParam)))
            return 0

        case UINT(WM_MOUSEWHEEL):
            dispatch(.mouseWheel(deviceId: .placeholder,
        ...
        case UINT(WM_KEYDOWN), UINT(WM_SYSKEYDOWN):
            let ev = KeyEvent(physicalKey: UInt32((lParam >> 16) & 0xFF),
...
private func globalWndProc(_ hWnd: HWND?, _ msg: UINT, _ wParam: WPARAM, _ lParam: LPARAM) -> LRESULT {
    ...
    return MainActor.assumeIsolated { window.takeUnretainedValue().wndProc(msg, wParam, lParam) }
}
